...
def generate_ranking_table(scores):
        ...
        for i, s in enumerate(scores, 1):
            ...
            w.writerow(
                [
                    ...
                    o["character_arc"],
                    o["total"],
                    m.get("screenplay_words", 0),
                    m.get("book_mentions", 0),
                ]
            )
    ...
    with open(md_path, "w") as f:
        ...
        f.write(
            "| Rank | Character | Pers | Role | Motiv | Arc | **Total** | Words | Mentions |\n"
        )
        f.write(
            "|------|-----------|------|------|-------|-----|-----------|-------|----------|\n"
        )
        for i, s in enumerate(scores, 1):
            ...
            m = s.get("meta", {})
            f.write(
                f"| {i} | {s['character']} | {o['personality']} | {o['narrative_role']} | "
        ...
        with open(os.path.join(reports_dir, f"{safe_name}.md"), "w") as f:
            ...
            for source, src_scores in s.get("per_source", {}).items():
                ...
                if src_type == "screenplay":
                    ...
                    f.write(
                        f'- Dialogue paragraphs: {meta.get("dialogue_paragraphs", 0)}\n'
                    )
