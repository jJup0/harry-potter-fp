...
HOUSES = {
    ...
    "Hermione Granger": "Gryffindor", "Neville Longbottom": "Gryffindor",
    "Ginny Weasley": "Gryffindor", "Fred Weasley": "Gryffindor",
    "George Weasley": "Gryffindor", "Albus Dumbledore": "Gryffindor",
    "Rubeus Hagrid": "Gryffindor", "Sirius Black": "Gryffindor",
    "Remus Lupin": "Gryffindor", "Minerva McGonagall": "Gryffindor",
    "Arthur Weasley": "Gryffindor", "Molly Weasley": "Gryffindor",
    "Peter Pettigrew": "Gryffindor", "Seamus Finnigan": "Gryffindor",
    "Dean Thomas": "Gryffindor", "Lavender Brown": "Gryffindor",
    "Parvati Patil": "Gryffindor", "Colin Creevey": "Gryffindor",
    "Draco Malfoy": "Slytherin", "Severus Snape": "Slytherin",
    "Lord Voldemort": "Slytherin", "Lucius Malfoy": "Slytherin",
    "Bellatrix Lestrange": "Slytherin", "Narcissa Malfoy": "Slytherin",
    "Vincent Crabbe": "Slytherin", "Gregory Goyle": "Slytherin",
    "Horace Slughorn": "Slytherin", "Pansy Parkinson": "Slytherin",
    "Blaise Zabini": "Slytherin",
    "Luna Lovegood": "Ravenclaw", "Cho Chang": "Ravenclaw",
    "Padma Patil": "Ravenclaw", "Gilderoy Lockhart": "Ravenclaw",
    "Sybill Trelawney": "Ravenclaw",
    "Cedric Diggory": "Hufflepuff", "Nymphadora Tonks": "Hufflepuff",
...
FILM_ORDER = [
    ...
    "7_deathly_hallows_p1", "8_deathly_hallows_p2"
]
FILM_LABELS = {
    "1_philosophers_stone": "PS", "2_chamber_of_secrets": "CoS",
    "3_prisoner_of_azkaban": "PoA", "4_goblet_of_fire": "GoF",
    "5_order_of_the_phoenix": "OotP", "6_half_blood_prince": "HBP",
    "7_deathly_hallows_p1": "DH1", "8_deathly_hallows_p2": "DH2"
}
BOOK_ORDER = [
    "1_philosophers_stone", "2_chamber_of_secrets", "3_prisoner_of_azkaban",
    "4_goblet_of_fire", "5_order_of_the_phoenix", "6_half_blood_prince",
...
def fig_ranking_bar(scores, top_n=30):
    ...
    fig = go.Figure()
    colors = {"personality": "#e74c3c", "narrative_role": "#3498db",
              "motivations": "#2ecc71", "character_arc": "#f39c12"}
    ...
    fig.update_layout(
        barmode='stack', title=f'Top {top_n} Characters by Faithfulness Score',
        xaxis_title='Character', yaxis_title='FP Score (out of 100)',
        yaxis=dict(range=[0, 105]), height=500,
...
def fig_scatter_presence(scores):
    ...
    fig = px.scatter(
        df, x='book_mentions', y='screenplay_words', size='total_fp',
        color='house', hover_name='character',
        color_discrete_map=HOUSE_COLORS,
        title='Character Presence: Books vs Films (sized by FP score)',
        labels={'book_mentions': 'Book Mentions', 'screenplay_words': 'Screenplay Words'},
...
def fig_heatmap_screen_time(screen_time, top_n=25):
    ...
    sorted_chars = sorted(screen_time.items(), key=lambda x: x[1].get('_total', 0), reverse=True)[:top_n]
    ...
    fig = go.Figure(data=go.Heatmap(
        z=z, x=[FILM_LABELS.get(f, f) for f in FILM_ORDER], y=chars,
        colorscale='YlOrRd', hovertemplate='%{y}<br>%{x}: %{z} words<extra></extra>'
    ))
    fig.update_layout(
        title=f'Screen Time Heatmap — Top {top_n} Characters (words spoken)',
        height=max(400, top_n * 22), yaxis=dict(autorange='reversed')
    )
    return fig
...
def fig_heatmap_book_mentions(book_mentions, top_n=25):
    ...
    sorted_chars = sorted(book_mentions.items(), key=lambda x: x[1].get('_total', 0), reverse=True)[:top_n]
    chars = [c[0] for c in sorted_chars]
    ...
    fig = go.Figure(data=go.Heatmap(
        z=z, x=[BOOK_LABELS.get(b, b) for b in BOOK_ORDER], y=chars,
        colorscale='YlGnBu', hovertemplate='%{y}<br>%{x}: %{z} mentions<extra></extra>'
    ))
    fig.update_layout(
    ...
    for i, name in enumerate(char_names):
        ...
        if film_x:
            fig.add_trace(go.Scatter(
                x=film_x, y=film_y, mode='lines+markers', name=f'{name} (film)',
                line=dict(color=color), marker=dict(symbol='circle')
            ))
        if book_x:
            fig.add_trace(go.Scatter(
                x=book_x, y=book_y, mode='lines+markers', name=f'{name} (book)',
                line=dict(color=color, dash='dash'), marker=dict(symbol='diamond')
            ))

    fig.update_layout(
        title='Character FP Score Across Installments',
        xaxis_title='Installment', yaxis_title='FP Score (per source)',
        yaxis=dict(range=[0, 105]), height=500,
        legend=dict(orientation='h', yanchor='bottom', y=1.02)
    )
    return fig
...
<div class="stats">
  <div class="stat"><div class="stat-value">""" + str(len(scores)) + """</div><div class="stat-label">Characters Scored</div></div>
  ...
  <div class="stat"><div class="stat-value">""" + f"{scores[0]['overall']['total']:.1f}" + """</div><div class="stat-label">Highest FP Score</div></div>
</div>
"""]
