        ...
        for scene in data["scenes"]:
            for d in scene.get("dialogue", []):
                ...
                if not canonical:
                    # Try stripping title
                    stripped = re.sub(
                        r"^(professor|mr\.?|mrs\.?|uncle|aunt)\s+", "", speaker
                    ).strip()
                ...
                screen_time[canonical][film] = (
                    screen_time[canonical].get(film, 0) + words
                )
...
def main():
    ...
    # Sort by total
    screen_time = dict(
        sorted(screen_time.items(), key=lambda x: x[1].get("_total", 0), reverse=True)
    )
    ...
    mentions = dict(
        sorted(mentions.items(), key=lambda x: x[1].get("_total", 0), reverse=True)
    )
