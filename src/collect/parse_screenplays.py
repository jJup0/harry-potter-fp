...
def parse_fandom_transcript(text, char_names):
    ...
    for line in lines:
        ...
        if direction_match:
            ...
            if is_scene_break and (current_scene['dialogue'] or current_scene['directions']):
...
def main():
    ...
    for fname in sorted(os.listdir(SCREENPLAYS_DIR)):
        ...
        print(f"{fname}: {len(scenes)} scenes, {total_dialogue} dialogue lines, {len(total_chars)} characters")

        out_name = fname.replace('.txt', '.json')
        with open(os.path.join(OUTPUT_DIR, out_name), 'w') as f:
            json.dump({'film': fname.replace('.txt', ''), 'scenes': scenes}, f, indent=2)
