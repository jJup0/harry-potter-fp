...
def split_chapters(text):
    ...
    pattern = re.compile(
        r'^\s*-?\s*[Cc][Hh][Aa][Pp][Tt][Ee][Rr]\s+[A-Za-z0-9 \t\-:vI]+',
        re.MULTILINE
    )
...
def detect_dialogue_speakers(text, name_map):
    ...
    for match in pattern.finditer(text):
        ...
        # Try stripping title
        stripped = re.sub(r'^(professor|mr\.?|mrs\.?|ms\.?|sir|lord|lady|madam|uncle|aunt)\s+', '', name).strip()
...
def main():
    ...
    for fname in sorted(os.listdir(BOOKS_DIR)):
        ...
        print(f"{fname}: {len(chapters)} chapters, {total_scenes} paragraphs, {len(all_chars)} characters")

        out_name = fname.replace('.txt', '.json')
        with open(os.path.join(OUTPUT_DIR, out_name), 'w') as f:
            json.dump({'book': fname.replace('.txt', ''), 'chapters': chapters}, f, indent=2)
