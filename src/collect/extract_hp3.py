...
class Extractor(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if self.in_body and tag in ('p', 'br', 'li', 'h2', 'h3', 'td', 'tr'):
            if self.current.strip(): self.lines.append(self.current.strip())
            self.current = ''
    ...
    def handle_data(self, data):
        if self.in_body and self.skip == 0: self.current += data
...
with open('/Users/jroi/personal/harry-potter-aitor/data/raw/screenplays/3_prisoner_of_azkaban.txt', 'w') as f:
    f.write(text)
