...
BOOK_MENTIONS_FILE = os.path.join(PROJECT_ROOT, "data", "metrics", "book_mentions_v2.json")
...
def build_character_registry():
    ...
    for name in sorted(all_names):
        ...
        stripped = re.sub(r'^(Professor |Mr\. |Mrs\. |Sir |Lord |Madam |Uncle |Aunt )', '', name).strip()
        if stripped != name:
    ...
    characters.sort(key=lambda x: x['book_mentions'] + x['screen_time_minutes'], reverse=True)

    with open(CHARACTERS_FILE, 'w') as f:
        yaml.dump({'characters': characters}, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
def load_alias_map():
    ...
    alias_map = {}
    blocklist = {'the', 'a', 'an', 'of', 'and', 'in', 'to', 'jr.', 'sr.', 'tom', 'lily',
                 'james', 'rose', 'hugo', 'bill', 'charlie', 'percy', 'dean', 'lee',
                 'colin', 'dennis', 'katie', 'hannah', 'susan', 'ernie', 'justin',
                 'lavender', 'padma', 'pansy', 'blaise', 'marcus', 'oliver'}
...
def parse_v2_screenplay(text, alias_map):
    ...
    for line in lines:
        ...
        if speaker_match:
            ...
            if name not in ('CONTINUED', 'CUT TO', 'FADE IN', 'FADE OUT', 'THE END',
                            'MORE', 'CONT', 'DISSOLVE TO'):
...
def parse_v1_screenplay(text, alias_map):
    ...
    for line in text.split('\n'):
        ...
        if direction_match:
            ...
            for alias, canonical in alias_map.items():
                if len(alias) >= 4 and re.search(r'\b' + re.escape(alias) + r'\b', direction, re.IGNORECASE):
                    current_scene['characters'].add(canonical.lower())
            continue
...
def split_chapters(text):
    pattern = re.compile(r'^\s*-?\s*[Cc][Hh][Aa][Pp][Tt][Ee][Rr]\s+[A-Za-z0-9 \t\-:vI]+', re.MULTILINE)
...
def build_corpus(alias_map):
    ...
    corpus = {}  # canonical -> {screenplays: {source: [scenes]}, books: {source: [scenes]}}
    ...
    for fname in sorted(os.listdir(sp_dir)):
        ...
        for scene_idx, scene in enumerate(data['scenes']):
            ...
            entry = {'source': film, 'scene_index': scene_idx,
            ...
            for scene in chapter['scenes']:
                for char in scene.get('characters_mentioned', []):
                    ...
                    corpus[char]['books'].setdefault(book, []).append({
                        'source': book, 'chapter': chapter['chapter_title'],
                        'text': scene['text'], 'has_dialogue': scene.get('has_dialogue', False),
                    })
    ...
    for char, data in corpus.items():
        ...
        for sub in ('screenplays', 'books'):
            ...
            with open(os.path.join(sub_dir, 'scenes.json'), 'w') as f:
                json.dump({'character': char, 'total_scenes': len(all_scenes), 'scenes': all_scenes}, f, indent=2)

    # Stats
    top = sorted(corpus.items(),
